...
for i in range(90, test_cases):
    ...
    ani = animation.FuncAnimation(fig, animate, init_func = init, frames = len(diagonals), interval = frame_duration, repeat = False)
    ...
    plt.pause((len(diagonals) + 1) * (frame_duration / 1000))
    plt.close()